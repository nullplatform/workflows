...
| **[runtime-lifecycle/](./runtime-lifecycle)** | Weekly AWS Lambda runtime deprecation tracking: an agent scrapes the AWS runtime tables into hidden org metadata (lake-joinable), a scanner opens action items for scopes on deprecated/soon-deprecated runtimes with one-click migration suggestions (env-aware: prod is apply-only), an event router applies approved migrations via the progressive deploy and gates human "resolved" claims against the LIVE deployment, and a daily closer retires items once a compliant deploy is active. | Change-detected catalog writes (a metadata update means the catalog really changed), deterministic min-supported target picking with a 180-day horizon, flat per-item writes via engine `spreadItem` with per-iteration retry (no sub-workflow fan-out), per-suite [AI-suggestion UX](./docs/ai-suggestions-ux.md) conventions. |
