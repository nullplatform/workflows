#!/usr/bin/env node
/**
 * Uploads the lib-inventory workflow definitions to the engine.
 *
 * Same contract as the runtime-lifecycle uploader it is adapted from:
 *   NP_API_KEY=… pnpm tsx workflows/lib-inventory/setup/03-upload-workflows.mjs
 *   pnpm tsx workflows/lib-inventory/setup/03-upload-workflows.mjs --dry-run
 *
 ...
 *
 * Run `node scripts/build-workflow.mjs --check` first: the scan step's code is
 * generated from scanner/, and uploading a stale YAML ships stale scanner code.
 */
...
// ORDER MATTERS: wf-l0 first. Its callers reference it by the client key
// `lib_inventory_scan_build`, which `patchReferences` rewrites to the per-org
// `wf_…` id — and it can only do that once wf-l0 has one.
const FILES = [
  { file: 'wf-l0-scan-build.yaml', key: 'lib_inventory_scan_build' },
  { file: 'wf-l1-backfill.yaml', key: 'lib_inventory_backfill' },
  { file: 'wf-l2-on-release.yaml', key: 'lib_inventory_on_release' },
];
