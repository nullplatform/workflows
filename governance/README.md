...
| `checklist-demo.yaml` | — (creates its own template) | Self-contained human-in-the-loop sandbox: creates a 2-item template (`np-checklist-create`), associates it with an approval action, waits on the outcome (`np-checklist-wait`) and branches on it. |
