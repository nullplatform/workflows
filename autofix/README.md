...
## What a reviewer sees on the item

The fix PR as a banner at the very top of the description
(`> 🔧 Fix pull request: #42 …`), written by the fixer when the PR opens and
preserved by every later refresh; the outcome as a comment (PR link,
`already_fixed`, or the reason a fix failed); and the close reason as the final
comment when a later build no longer reports the finding.
