...
describe('finops/tool-cloud-query', () => {
  it('returns results keyed by call id', async () => {
    ...
    // the action context carries no artifact any more: the platform derives it from package.image
    expect((seen[0]?.action_context as { notification?: unknown }).notification).toBeUndefined();
  });
