...
| `np-finops-worker-assume-policy.json` | the worker role, multi-account | lets the base identity assume `np-finops` in the LISTED accounts only, with the ExternalId (add one ARN per account; never `::*:`) |
